//$Id: drupalchat.js,v 1.14.2.1 2011/01/07 18:43:24 darklrd Exp $
var drupalchat = {
	username: null,
	uid: null,
	...
	YUI().use('gallery-storage-lite', function (Y) {
	    Y.StorageLite.on('storage-lite:ready', function () {
	        if(Y.StorageLite.getItem('username')!=null) {
	        	...
	        	if(Y.StorageLite.getItem('drupalchat').length) {
	...
	if((!drupalchat_messages.status || drupalchat_messages.status == 0)) {
		...
		$.each(drupalchat_messages.messages, function(index, value) {
			...
			if ($("#chatbox_"+chatboxtitle+" .chatboxcontent .chatboxusername a:last").html() == value.name) {
				...
				$("#chatbox_"+chatboxtitle+" .chatboxcontent").append('<div class="chatboxusername"><span class="chatboxtime">'+hours+':'+minutes+'</span><a href="'+Drupal.settings.basePath+'user/'+chatboxtitle+'">'+value.name+'</a></div><p>'+value.message+'</p>');
			}
	...
	if(event.keyCode == 13 && event.shiftKey == 0)  {
		...
		if (message != '') {
			...
			if ($("#chatbox_"+chatboxtitle+" .chatboxcontent .chatboxusername a:last").html() == Drupal.settings.drupalchat.username) {
				...
				$("#chatbox_"+chatboxtitle+" .chatboxcontent").append('<div class="chatboxusername"><span class="chatboxtime">'+hours+':'+minutes+'</span><a href="'+Drupal.settings.basePath+'user/'+Drupal.settings.drupalchat.uid+'">'+Drupal.settings.drupalchat.username+'</a></div><p>'+message+'</p>');
			}
